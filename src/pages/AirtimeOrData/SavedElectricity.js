    ...
    return (
        <FLexer height={300}>
            <IFlexer flexDirection="column" justifyContent='flex-start' >
                <TouchableOpacity
                    onPress={() => {
                        navigation.navigate('Electricity Form')
                    }}
                >
                    <Box
                        w={70}
                ...
                <IIText type='B' textAlign='center'>You have not saved any {"\n"}
                    Electricity details. Pay for Electricity to proceed
                </IIText>
                <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => {
                ...
                >
                    <Box
                        ...
                        marginTop={10}
                    >
                        <IIText type='B' color={Colors.DEFAULT} size={16}>Pay Electricity</IIText>
                    </Box>
                </TouchableOpacity>
            </IFlexer>
        </FLexer>
...
const SavedElectricity = () => {
    ...
    const { isLoading, mutate, isError } = useMutation(getAllPhoneBooks, {
        onSuccess: async data => {
            ...
            if (data?.data?.length) {
                const meters = data?.data?.filter(element => element.book_type == 'Meter')
    ...
    const handleFetch = () => {
        ...
        setMsg('Loading your saved meters, please wait...')
        mutate({ type: 'fetch' })
    }
    return (
        <View style={{ height: Dimensions.get('window').height - 250, justifyContent: 'space-between' }}>
            {
                savedMeters.length ? <DataState meters={savedMeters} /> : <EmptyState />
            }
            <IFlexer
                ...
                <Popover
                    ...
                    <Box>
                        ...
                        >
                            ...
                            >
                                <IIText type='B' color={Colors.DEFAULT}>New Meter</IIText>
...
export default SavedElectricity